...
2. **`publish`** — manual: promotes a chosen RC to a stable release (and npm, once configured).
...
push to develop ──► rc.yml ──► GitHub prerelease  v0.0.0-rc.<run#>
                      ...
                      Actions → publish.yml (manual dispatch)
                                    │
                        v0.0.0  stable release + npm publish (gated)
```
...
1. `npm ci` and build the framework (`npm run build --workspace spindle`)
...
4. `npm pack` → attach `spindle-<rc>.tgz`
...
4. **npm publish** — *gated*: skipped until the `NPM_TOKEN` repository secret is configured, then publishes `spindle@<final>` as `latest`
...
| `NPM_TOKEN` repo secret | Not configured — npm publish is stubbed until this exists |
